...
interface StatusStylePanelProps {
  workspace: Workspace
  t: Translator
  onConfigureStatus: (status: NoteStatus, config: StatusStyleConfig | undefined) => void
  onClose: () => void
}
...
export function StatusStylePanel({
  ...
  const statusStyles = workspace.statusStyles ?? {}

  return (
    <div className="status-style-overlay">
      <section className="status-style-panel" role="dialog" aria-modal="true">
        <header className="status-style-header">
          <h2>{t('statusStyleTitle')}</h2>
        ...
        <p className="status-style-intro">{t('statusStyleIntro')}</p>
        ...
        <div className="status-style-rows">
          {noteStatuses.map((status) => {
            ...
            return (
              <div key={status} className="status-style-row">
                <span className="status-style-status">{getNoteStatusLabel(status, t)}</span>

                <div className="status-style-configs">
                  <div className="status-style-field">
                    <span className="status-style-field-label">{t('color')}</span>
                      ...
                      {colorOptions.map((color) => (
                        <button
                          ...
                          className={`note-style-option note-style-color-${color} ${
                            config?.color === color ? 'note-style-option-selected' : ''
                          }`}
                          onClick={() =>
                            onConfigureStatus(status, {
                              ...config,
                              color,
                            })
                          }
                  ...
                  <div className="status-style-field">
                    <span className="status-style-field-label">{t('pattern')}</span>
                    ...
                    <div className="note-style-options">
                      {patternOptions.map((pattern) => (
                        <button
                          ...
                          className={`note-style-option note-style-color-concrete note-pattern-${pattern} ${
                            config?.pattern === pattern ? 'note-style-option-selected' : ''
                          }`}
                          onClick={() =>
                            onConfigureStatus(status, {
                              ...config,
                      ...
                      {config?.pattern && (
                        <button
                          type="button"
                          title={t('statusStyleClearPattern')}
                          className="status-style-clear"
                          onClick={() =>
                            onConfigureStatus(status, {
                              color: config.color,

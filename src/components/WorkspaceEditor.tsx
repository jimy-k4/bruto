...
export function WorkspaceEditor({
  ...
  return (
    <aside className="note-editor workspace-editor" style={editorStyle}>
      <div className="editor-resize-handle" onMouseDown={onStartResize} />

      <div className="note-editor-header">
        <div>
          <p className="sidebar-label">{t('editWorkspace')}</p>

          <span className="note-editor-id">{t('projectInformation')}</span>
        </div>
      ...
      <div className="note-editor-content">
        ...
        <div className="editor-placeholder">
          ...
          <p>{projectName?.toUpperCase() || t('localProject')}</p>
        </div>
      </div>

      <div className="note-editor-actions">
        <button className="secondary-button" onClick={onClose} disabled={savingWorkspaceInfo}>
          {t('cancel')}
        </button>
        ...
        <button
          ...
          onClick={onSave}
          disabled={savingWorkspaceInfo}
        >
          {savingWorkspaceInfo ? t('saving') : t('updateWorkspace')}
        </button>
      </div>
    </aside>
  )

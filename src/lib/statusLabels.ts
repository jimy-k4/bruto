...
export function getNoteStyle(
  ...
  workspace?: Pick<Workspace, 'statusStyles'> | null,
): {
  color: NoteColorTheme
  pattern: NotePattern
} {
  const config: StatusStyleConfig | undefined = note.status
    ? workspace?.statusStyles?.[note.status]
    : undefined

  return {
    color: config?.color ?? note.colorTheme,
    pattern: config?.pattern ?? note.pattern,
  }
}

export function getNoteStatusLabel(status: NoteStatus, t: Translator) {
  switch (status) {
    case 'idea':
      return t('statusIdea')
    case 'todo':
